...
public interface DatesRepository extends JpaRepository<Dates, Integer> {
    ...
    @Query(value = "SELECT * from dates where (full_date between adddate(Current_date(), INTERVAL 1 DAY) and adddate(Current_date(), INTERVAL 14 DAY))" +
            "and (day_of_week != 7 and day_of_week != 1)", nativeQuery = true)
